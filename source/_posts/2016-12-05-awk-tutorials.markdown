...
在`awk`操作里面如果要[转义][link_02]单引号`'`，则需要通过`'\''`来实现，上面这个示例中使用了两次单引号`'`。另外，使用了`printf`对数值进行[千分位格式化][link_03]，可以通过<a class='.blog-link' href='/assets/awk-tutorials/data.txt.7z'>链接</a>下载脚本使用的数据源文件。
